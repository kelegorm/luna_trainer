...
Все 28 требований origin переносятся как R-IDs и реализуются через unit-ы (см. таблицу ниже). Список техник MVP-каталога: `PairCompletion`, `TrioAvoidance`, `ParityFill`, `SignPropagation`, `AdvancedMidLineInference`, `ChainExtension`, `Composite(unknown)` (R2).

| Group | R-IDs |
|---|---|
| Solver & catalog | R1, R2 |
| Drill mechanics | R3, R4, R5, R6 |
| Mastery & scheduler | R7, R8, R9, R10, R11 |
| Hint ladder | R12, R13 |
| Telemetry & contamination | R14, R15, R16 |
| Replay-as-tutor | R17, R18 |
...
**Origin acceptance examples:** AE1 (R3, R6), AE2 (R12, R13), AE3 (R10), AE4 (R5), AE5 (R15), AE6 (R17), AE7 (R22, R23, R24), AE8 (R25).
...
- U7. **ContaminationDetector + telemetry pipeline**

**Goal:** Сервис, объединяющий AppLifecycleListener и sensors_plus motion-detection в один stream `MoveTimingMetadata`, который пишется в `MoveEventsRepository` при коммите хода.

**Requirements:** R14, R15, R16 (timer hidden — не показываем wall-clock пользователю).
...
**Verification:**
- Все тесты зелёные. Manual smoke на Android: открыть test screen, сделать backgrounding и положить телефон — оба сигнала видны в debug-overlay.
...
Telemetry pipeline + mastery + FSRS + drill selector. Готовность: можно симулировать сессию через unit-тесты, увидеть, что mastery апдейтится, FSRS-due двигается.

### Phase D — UX & Integration (U10, U11, U12, U13)

Полный playable MVP. Готовность: на Android можно сыграть партию, получить summary, начать drill, увидеть Mastery-экран.
